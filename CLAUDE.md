...
-   **Presets**: `low` (VM only), `medium` (+preprocess, encrypt, rolling cipher, decoy/dynamic opcodes), `max` (+debug protection, integrity binding, dead code, stack encoding, VM shielding, MBA, handler fragmentation — note: handler fragmentation auto-disabled by function table dispatch). Defined in `presets.ts`.
...
-   **Per-build structural variation** (`structural-choices.ts`): A `StructuralChoices` object is derived from the build seed via a separate LCG stream (`seed ^ 0xDEADBEEF`) so it doesn't perturb existing PRNG sequences. Controls: (1) runtime statement order shuffling within 5 dependency tiers (~6.2M orderings), (2) declaration style variation (individual/chained/mixed `var` statements), (3) expression noise (dot↔bracket notation, `===`↔`!(  !==)`, numeric literal computed equivalents), all via per-node coin flips. Applied as a post-processing AST walk in `ruamvm/structural-transforms.ts` after all builders produce their nodes. All transforms are semantics-preserving.
