 ...
 */
export function generateFingerprintSource(): string {
  return `function _fingerprint(){var h=0x5f3759df;h^=Array.prototype.reduce.length<<0x18;h^=String.prototype.charCodeAt.length<<0x14;h^=Math.floor.length<<0x10;h^=Object.keys.length<<0x0c;h^=JSON.stringify.length<<0x08;h^=parseInt.length<<0x04;h=(h^(h>>>16))*0x45d9f3b;h=(h^(h>>>13))*0x45d9f3b;h=h^(h>>>16);return h>>>0;}`;
}
