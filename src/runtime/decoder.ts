...
export function generateDecoderSource(): string {
  return `
function _rc4(data,key){var S=new Array(256);var j=0;var i;for(i=0;i<256;i++)S[i]=i;for(i=0;i<256;i++){j=(j+S[i]+key.charCodeAt(i%key.length))&255;var t=S[i];S[i]=S[j];S[j]=t;}i=0;j=0;var out=new Uint8Array(data.length);for(var k=0;k<data.length;k++){i=(i+1)&255;j=(j+S[i])&255;var t=S[i];S[i]=S[j];S[j]=t;out[k]=data[k]^S[(S[i]+S[j])&255];}return out;}
function _b64decode(str){if(typeof atob==='function'){var bin=atob(str);var bytes=new Uint8Array(bin.length);for(var i=0;i<bin.length;i++)bytes[i]=bin.charCodeAt(i);return bytes;}else{return new Uint8Array(Buffer.from(str,'base64'));};}
`.trim();
}
