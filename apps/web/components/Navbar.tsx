...
const links = [
  { label: "docs", href: "https://github.com/owengregson/ruam#readme", icon: faBook },
  { label: "github", href: "https://github.com/owengregson/ruam", icon: faGithub },
    ...
    <header className="fixed top-0 z-40 w-full border-b border-edge/60 bg-void/80 backdrop-blur-xl">
      <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-6">
        ...
        <a href="#" className="flex items-center gap-2.5">
          <Image src="/ruam.svg" alt="Ruam" width={26} height={26} />
          <span className="font-display text-lg italic text-snow">ruam</span>
        ...
        <nav className="hidden items-center gap-1 md:flex">
          {links.map((l, i) => (
            <span key={l.label} className="flex items-center">
              {i > 0 && (
                <span className="mx-2 font-mono text-xs text-steel">/</span>
              )}
              <a
                href={l.href}
                target="_blank"
              ...
              >
                <FontAwesomeIcon icon={l.icon} className="h-3 w-3" />
                {l.label}
              </a>
            </span>
          ))}
