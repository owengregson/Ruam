...
**Spec:** `docs/specs/2026-03-10-codegen-ast-builder-design.md`
...
**Files:**
- Create: `packages/ruam/src/codegen/nodes.ts`
...
- [ ] **Step 1: Create the codegen directory**

```bash
mkdir -p packages/ruam/src/codegen/handlers packages/ruam/src/codegen/builders
```

- [ ] **Step 2: Write `nodes.ts` with all node types and factory functions**

```typescript
// packages/ruam/src/codegen/nodes.ts
/**
...
```bash
git add packages/ruam/src/codegen/nodes.ts
git commit -m "feat(codegen): add AST node types and factory functions"
```
...
**Files:**
- Create: `packages/ruam/src/codegen/emit.ts`
...
Refer to:
- `packages/ruam/src/codegen/nodes.ts` — for all node type definitions
...
```typescript
// packages/ruam/src/codegen/emit.ts
/**
...
function needsParens(child: JsNode, parentOp: string, isRight: boolean): boolean {
  let cp: number;
  if (child.type === 'BinOp') cp = PREC[child.op] ?? 0;
  else if (child.type === 'AssignExpr') cp = PREC[child.op ? child.op + '=' : '='] ?? 2;
...
export function emit(node: JsNode): string {
  switch (node.type) {
    ...
    case 'ConstDecl':
      return `const ${node.name}${node.init ? '=' + emit(node.init) : ''}`;
    case 'FnDecl':
      return `${node.async ? 'async ' : ''}function ${node.name}(${node.params.join(',')}){${emitBody(node.body)}}`;
    ...
    case 'IfStmt':
      return `if(${emit(node.test)}){${emitBody(node.then)}}${node.else ? `else{${emitBody(node.else)}}` : ''}`;
    case 'WhileStmt':
      return `while(${emit(node.test)}){${emitBody(node.body)}}`;
    case 'ForStmt':
      return `for(${node.init ? emit(node.init) : ''};${node.test ? emit(node.test) : ''};${node.update ? emit(node.update) : ''}){${emitBody(node.body)}}`;
    case 'ForInStmt':
      return `for(var ${node.decl} in ${emit(node.obj)}){${emitBody(node.body)}}`;
    case 'SwitchStmt':
      return `switch(${emit(node.disc)}){${node.cases.map(c => emit(c)).join('')}}`;
    case 'CaseClause':
    ...
    case 'TryCatchStmt': {
      ...
      if (node.handler) {
        s += node.param ? `catch(${node.param}){${emitBody(node.handler)}}` : `catch{${emitBody(node.handler)}}`;
      }
      if (node.finalizer) s += `finally{${emitBody(node.finalizer)}}`;
      return s;
    }
    case 'DebuggerStmt': return 'debugger;';

    // --- Expressions ---
    case 'Id': return node.name;
    case 'Literal': return emitLiteral(node.value);
    case 'BinOp': {
      const left = needsParens(node.left, node.op, false) ? `(${emit(node.left)})` : emit(node.left);
      const right = needsParens(node.right, node.op, true) ? `(${emit(node.right)})` : emit(node.right);
      if (KEYWORD_BINOP.has(node.op)) return `${left} ${node.op} ${right}`;
      return `${left}${node.op}${right}`;
    }
    case 'UnaryOp':
      if (KEYWORD_UNOP.has(node.op)) return `${node.op} ${emit(node.expr)}`;
      // Prevent --x from becoming ---x (double minus + negate)
      if (node.op === '-' && node.expr.type === 'UnaryOp' && node.expr.op === '-') return `-(${emit(node.expr)})`;
      return `${node.op}${emit(node.expr)}`;
    case 'UpdateExpr':
      return node.prefix ? `${node.op}${emit(node.arg)}` : `${emit(node.arg)}${node.op}`;
    case 'AssignExpr':
    ...
    case 'CallExpr': {
      ...
      if (node.callee.type === 'FnExpr' || node.callee.type === 'ArrowFn') callee = `(${callee})`;
    ...
    case 'ObjectExpr':
      return `{${node.entries.map(([k, v]) =>
        `${typeof k === 'string' ? k : `[${emit(k)}]`}:${emit(v)}`
      ).join(',')}}`;
    case 'FnExpr':
      return `${node.async ? 'async ' : ''}function${node.name ? ' ' + node.name : ''}(${node.params.join(',')}){${emitBody(node.body)}}`;
    case 'ArrowFn': {
      const params = node.params.length === 1 && !node.params[0]!.startsWith('...') && !node.async
        ? node.params[0]!
        : `(${node.params.join(',')})`;
      let body: string;
      if (node.body.length === 1 && node.body[0]!.type === 'ReturnStmt' && (node.body[0] as ReturnStmt).value) {
    ...
    case 'NewExpr':
      return `new ${emit(node.callee)}(${node.args.map(a => emit(a)).join(',')})`;
    case 'SequenceExpr':
...
function emitObj(obj: JsNode): string {
  ...
  if (obj.type === 'Literal' && typeof obj.value === 'number') return `(${s})`;
...
```bash
git add packages/ruam/src/codegen/emit.ts
git commit -m "feat(codegen): add recursive AST-to-JS emitter"
```
...
**Files:**
- Create: `packages/ruam/test/codegen/emit.test.ts`
...
```typescript
// packages/ruam/test/codegen/emit.test.ts
...
Run: `cd packages/ruam && npx vitest run test/codegen/emit.test.ts`
Expected: All tests pass

- [ ] **Step 3: Commit**

```bash
git add packages/ruam/test/codegen/emit.test.ts
git commit -m "test(codegen): add comprehensive emitter unit tests"
```
...
**Files:**
- Create: `packages/ruam/src/codegen/handlers/index.ts`
...
```typescript
// packages/ruam/src/codegen/handlers/index.ts
/**
...
```bash
git add packages/ruam/src/codegen/handlers/index.ts
git commit -m "feat(codegen): add HandlerCtx type and handler registry"
```
...
**Files:**
- Create: `packages/ruam/src/codegen/handlers/stack.ts`
- Modify: `packages/ruam/src/codegen/handlers/index.ts` — import and register
...
```typescript
// packages/ruam/src/codegen/handlers/stack.ts
import { Op } from "../../compiler/opcodes.js";
import { id, call, lit, index, bin, assign, update, raw, breakStmt } from "../nodes.js";
import type { HandlerCtx, HandlerFn } from "./index.js";
import { registry } from "./index.js";

function PUSH_CONST(ctx: HandlerCtx) { /* return [call W with C[O], breakStmt()] */ }
function PUSH_UNDEFINED(ctx: HandlerCtx) { /* W(void 0) */ }
...
```bash
git add packages/ruam/src/codegen/handlers/stack.ts packages/ruam/src/codegen/handlers/index.ts
git commit -m "feat(codegen): extract stack manipulation opcode handlers"
```
...
**Files:**
- Create: `packages/ruam/src/codegen/handlers/arithmetic.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/handlers/comparison.ts`
- Create: `packages/ruam/src/codegen/handlers/logical.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/handlers/control-flow.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/handlers/registers.ts`

**Context:** Simple load/store to register array R[O] or args A[O]. The compound ops (INC_REG, POST_INC_REG, *_ASSIGN_REG) are straightforward. FAST_* are inlined versions.
...
**Files:**
- Create: `packages/ruam/src/codegen/handlers/type-ops.ts`
- Create: `packages/ruam/src/codegen/handlers/special.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/handlers/destructuring.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/handlers/superinstructions.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/handlers/scope.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/handlers/compound-scoped.ts`
...
```typescript
function compoundScoped(ctx: HandlerCtx, op: string): JsNode[] { /* scope walk + compound assign */ }
```
...
**Files:**
- Create: `packages/ruam/src/codegen/handlers/objects.ts`
...
- [ ] **Step 1: Write object handlers** (all GET_PROP_*, SET_PROP_*, DELETE_PROP_*, OPT_CHAIN_*, IN_OP, INSTANCEOF, GET_SUPER_PROP, SET_SUPER_PROP, GET/SET/HAS_PRIVATE_FIELD, DEFINE_OWN_PROPERTY, NEW_OBJECT, NEW_ARRAY, NEW_ARRAY_WITH_SIZE, ARRAY_PUSH, ARRAY_HOLE, SPREAD_ARRAY, SPREAD_OBJECT, COPY_DATA_PROPERTIES, SET_PROTO, FREEZE_OBJECT, SEAL_OBJECT, DEFINE_PROPERTY_DESC, CREATE_TEMPLATE_OBJECT)
- [ ] **Step 2: Register, compile, commit**
...
**Files:**
- Create: `packages/ruam/src/codegen/handlers/calls.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/handlers/classes.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/handlers/exceptions.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/handlers/iterators.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/handlers/generators.ts`
...
- [ ] **Step 1: Write generator handlers** (YIELD, YIELD_DELEGATE, AWAIT, CREATE_GENERATOR, GENERATOR_RESUME/RETURN/THROW, SUSPEND, RESUME, ASYNC_GENERATOR_*)
- [ ] **Step 2: Register, compile, commit**
...
**Files:**
- Create: `packages/ruam/src/codegen/handlers/functions.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/builders/interpreter.ts`
...
- `packages/ruam/src/codegen/handlers/index.ts` — registry and HandlerCtx

- [ ] **Step 1: Write `builders/interpreter.ts`**

```typescript
// packages/ruam/src/codegen/builders/interpreter.ts
import type { JsNode } from "../nodes.js";
import type { RuntimeNames } from "../../runtime/names.js";
import type { InterpreterOptions } from "../../runtime/templates/interpreter.js";
import { fn, varDecl, id, lit, raw, switchStmt, caseClause, breakStmt, block } from "../nodes.js";
...
export function buildInterpreter(
  ...
  for (const [op, handler] of registry) {
    if (opts.interpOpts.dynamicOpcodes && opts.interpOpts.usedOpcodes && !opts.interpOpts.usedOpcodes.has(op)) continue;
  ...
  if (opts.interpOpts.decoyOpcodes && opts.interpOpts.usedOpcodes) {
    ...
    cases.push(...generateDecoyHandlers(ctx, shuffleMap, opts.interpOpts.usedOpcodes));
  }
...
**Files:**
- Create: `packages/ruam/src/codegen/transforms.ts`
...
```typescript
// packages/ruam/src/codegen/transforms.ts
import type { JsNode } from "./nodes.js";
 ...
 */
export function inlineStackOps(nodes: JsNode[], S: string, P: string, W: string, X: string, Y: string): JsNode[] {
...
**Files:**
- Modify: `packages/ruam/src/codegen/transforms.ts`
- Create: `packages/ruam/test/codegen/transforms.test.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/builders/globals.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/builders/fingerprint.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/builders/rolling-cipher.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/builders/decoder.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/builders/loader.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/builders/runners.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/builders/deserializer.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/builders/stack-encoding.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/builders/debug-protection.ts`
...
**Files:**
- Create: `packages/ruam/src/codegen/builders/debug-logging.ts`
...
- All builder files in `packages/ruam/src/codegen/builders/`

- [ ] **Step 1: Update `generateVmRuntime()` to use builders**

```typescript
import { emit } from "../codegen/emit.js";
import { iife, block, raw, exprStmt, varDecl, id, lit } from "../codegen/nodes.js";
import { buildInterpreter } from "../codegen/builders/interpreter.js";
// ... import all builders
...
git commit -m "feat(codegen): switch vm.ts orchestrator to AST builder"
```
...
Run: `grep -r "from.*templates/" packages/ruam/src/ --include="*.ts" | grep -v codegen`
...
**Files:**
- Modify: `CLAUDE.md` — update project structure section, remove `runtime/templates/` references, add `codegen/` section
- Modify: memory files — update architecture notes

- [ ] **Step 1: Update CLAUDE.md**

Update the project structure to reflect the new `codegen/` directory and removal of `runtime/templates/`. Update the "Runtime templates" convention to describe the AST builder system instead.
