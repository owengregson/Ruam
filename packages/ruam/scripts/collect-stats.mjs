...
fibonacci(10);`;
  ...
  for (const w of workloads) {
    ...
    console.log(
      `    ${w.name}: ${multiplier.toFixed(1)}x (native: ${nativeMs.toFixed(3)}ms, VM: ${vmMs.toFixed(3)}ms)`,
    );
  }
  ...
  return {
    ...
    size: {
      ...
      low: { bytes: lowBytes, ratio: +(lowBytes / inputBytes).toFixed(1) },
      medium: { bytes: medBytes, ratio: +(medBytes / inputBytes).toFixed(1) },
      high: { bytes: highBytes, ratio: +(highBytes / inputBytes).toFixed(1) },
    },
  };
}
...
console.log(
  `  Tests code:   ${source.testFiles} files, ${fmtNum(source.testLines)} lines`,
);
console.log(
  `  Total:        ${source.totalFiles} files, ${fmtNum(source.totalLines)} lines`,
);
...
if (!tests && prev?.tests) {
  tests = prev.tests;
  console.log(
    `  Tests:        ${fmtNum(tests.passed)}/${fmtNum(tests.total)} passing (cached)`,
  );
} else if (tests) {
  console.log(
...
if (runBench) {
  ...
  if (bench) {
    ...
    console.log(
      `  Size (low preset):    ${sizeData.low.ratio}x (${fmtNum(sizeData.low.bytes)} bytes)`,
    );
    console.log(
      `  Size (high preset):   ${sizeData.high.ratio}x (${fmtNum(sizeData.high.bytes)} bytes)`,
    );
  }
} else {
  perfData = prev?.performance ?? null;
  sizeData = prev?.size ?? null;
  if (perfData) console.log(`  Performance:  ${perfData.weightedAvg}x weighted avg (cached)`);
  if (sizeData) console.log(`  Size (low):   ${sizeData.low.ratio}x ratio (cached)`);
}
