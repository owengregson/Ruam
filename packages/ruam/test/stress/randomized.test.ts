...
describe("randomized: string operations", () => {
	for (let trial = 0; trial < 8 * FUZZ_MULTIPLIER; trial++) {
		...
		const needle = s.substring(
			randInt(0, 3),
			randInt(3, Math.min(6, s.length))
		);
	...
	for (let trial = 0; trial < 5 * FUZZ_MULTIPLIER; trial++) {
		...
		it(`substring/slice on "${str.substring(
			0,
			8
		)}..." [${start}:${end}]`, () => {
			assertEquivalent(`
...
describe("randomized: array operations", () => {
	...
	for (let trial = 0; trial < 5 * FUZZ_MULTIPLIER; trial++) {
        ...
        function f() {
          var arr = [${arr.join(",")}];
          var removed = arr.splice(${spliceStart}, ${spliceDelete}${
				spliceInsert.length > 0 ? ", " + spliceInsert.join(",") : ""
			});
          return [arr, removed];
        }
        f();
...
describe("randomized: object operations", () => {
	for (let trial = 0; trial < 8 * FUZZ_MULTIPLIER; trial++) {
		...
		for (let i = 0; i < numKeys; i++)
			keys.push("k" + randString(randInt(2, 5)));
...
describe("randomized: switch", () => {
	for (let trial = 0; trial < 5 * FUZZ_MULTIPLIER; trial++) {
		...
		const testValue = randBool()
			? caseValues[randInt(0, numCases - 1)]!
			: randInt(0, 20);

		const cases = caseValues
			.map((v, i) => `case ${v}: return ${i + 1};`)
			.join("\n            ");
