...
(function testLogical() {
	...
	assert("short circuit and", false && (function () {})(), false);
})();
...
(function testArrays() {
	...
	assert(
		"array sort custom",
		(function () {
			return [3, 1, 2].sort(function (a, b) {
				return b - a;
			})[0];
		})(),
		3
...
(function testLoops() {
	...
	assert(
		...
		(function () {
			var s = 0;
			for (var i = 0; i < 3; i++) for (var j = 0; j < 3; j++) s++;
			return s;
		})(),
		9
	);
	...
	assert(
		...
		(function () {
			var s = "";
			for (var c of "abc") s += c.toUpperCase();
			return s;
		})(),
		"ABC"
	);
	...
	assert(
		...
		(function () {
			function qsort(arr) {
				...
				var pivot = arr[0];
				var left = arr.slice(1).filter(function (x) {
					return x <= pivot;
				});
				var right = arr.slice(1).filter(function (x) {
					return x > pivot;
				});
...
(function testEdgeCases() {
	...
	assert(
		"conditional chain",
		(function (x) {
			return x > 0 ? (x > 10 ? "big" : "small") : "non-positive";
		})(5),
		"small"
	);
	assert(
