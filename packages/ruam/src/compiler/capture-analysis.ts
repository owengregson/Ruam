 ...
 */
export function analyzeCapturedVars(fnPath: NodePath<t.Function>): CaptureAnalysisResult {
...
function collectParamNames(param: NodePath<t.LVal>, vars: Map<string, VarInfo>, argIndex: number): void {
  if (param.isIdentifier()) {
    vars.set(param.node.name, { name: param.node.name, isCaptured: false, register: -1, kind: "param" });
  } else if (param.isAssignmentPattern()) {
    const left = param.get("left");
    if (left.isIdentifier()) {
      vars.set(left.node.name, { name: left.node.name, isCaptured: false, register: -1, kind: "param" });
    } else {
    ...
    if (arg.isIdentifier()) {
      vars.set(arg.node.name, { name: arg.node.name, isCaptured: false, register: -1, kind: "param" });
    }
...
function collectPatternNames(pattern: NodePath<t.LVal>, vars: Map<string, VarInfo>, kind: VarInfo["kind"]): void {
  if (pattern.isIdentifier()) {
    vars.set(pattern.node.name, { name: pattern.node.name, isCaptured: false, register: -1, kind });
  } else if (pattern.isArrayPattern()) {
    for (const elem of pattern.get("elements")) {
      if (elem.node !== null) {
        if (elem.isRestElement()) {
          collectPatternNames(elem.get("argument") as NodePath<t.LVal>, vars, kind);
        } else {
    ...
    for (const prop of pattern.get("properties")) {
      if (prop.isObjectProperty()) {
        collectPatternNames(prop.get("value") as NodePath<t.LVal>, vars, kind);
      } else if (prop.isRestElement()) {
        collectPatternNames(prop.get("argument") as NodePath<t.LVal>, vars, kind);
      }
    }
  } else if (pattern.isAssignmentPattern()) {
    collectPatternNames(pattern.get("left") as NodePath<t.LVal>, vars, kind);
  }
}
      ...
      for (const declarator of path.get("declarations")) {
        ...
        if (id.isIdentifier()) {
          if (vars.has(id.node.name)) {
            ...
            vars.set(id.node.name, { name: id.node.name, isCaptured: false, register: -1, kind });
          }
        } else {
    ...
    FunctionDeclaration(path) {
      ...
      if (path.node.id) {
        if (!vars.has(path.node.id.name)) {
          vars.set(path.node.id.name, { name: path.node.id.name, isCaptured: false, register: -1, kind: "var" });
        }
      }
    },
  });
  ...
  bodyPath.traverse({
    "FunctionDeclaration|FunctionExpression|ArrowFunctionExpression|ObjectMethod|ClassMethod"(
      ...
      if (innerBody.isBlockStatement()) {
        innerBody.traverse({
          VariableDeclaration(vpath: NodePath<t.VariableDeclaration>) {
            for (const d of vpath.get("declarations")) {
              const id = d.node.id;
              collectNodeNames(id, locallyDeclared);
            }
          },
          FunctionDeclaration(fpath: NodePath<t.FunctionDeclaration>) {
            if (fpath.node.id) locallyDeclared.add(fpath.node.id.name);
          },
        });
      }
...
function collectNodeNames(node: t.Node, names: Set<string>): void {
  if (node.type === "Identifier") {
    ...
    for (const prop of (node as t.ObjectPattern).properties) {
      if (prop.type === "ObjectProperty") collectNodeNames(prop.value as t.Node, names);
      else if (prop.type === "RestElement") collectNodeNames(prop.argument, names);
    }
...
function isVariableReference(path: NodePath<t.Identifier>): boolean {
  ...
  if (parent.type === "ObjectProperty" && key === "key" && !parent.computed) return false;
  // Object method key
  if (parent.type === "ObjectMethod" && key === "key" && !parent.computed) return false;
  // Class method key
  if (parent.type === "ClassMethod" && key === "key" && !parent.computed) return false;
