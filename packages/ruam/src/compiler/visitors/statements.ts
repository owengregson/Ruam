	...
	switch (node.type) {
		...
		case "ReturnStatement":
			compileReturnStatement(
				path as NodePath<t.ReturnStatement>,
				emitter,
				scope,
				ctx
			);
		...
		case "IfStatement":
			compileIfStatement(
				path as NodePath<t.IfStatement>,
				emitter,
				scope,
				ctx,
				loopStack
		...
		case "BlockStatement":
			compileBlockStatement(
				path as NodePath<t.BlockStatement>,
				emitter,
				scope,
				ctx,
				loopStack
		...
		case "ForStatement":
			compileForStatement(
				path as NodePath<t.ForStatement>,
				emitter,
				scope,
				ctx,
				loopStack
		...
		case "WhileStatement":
			compileWhileStatement(
				path as NodePath<t.WhileStatement>,
				emitter,
				scope,
				ctx,
				loopStack
		...
		case "DoWhileStatement":
			compileDoWhileStatement(
				path as NodePath<t.DoWhileStatement>,
				emitter,
				scope,
				ctx,
				loopStack
		...
		case "ForInStatement":
			compileForInStatement(
				path as NodePath<t.ForInStatement>,
				emitter,
				scope,
				ctx,
				loopStack
		...
		case "ForOfStatement":
			compileForOfStatement(
				path as NodePath<t.ForOfStatement>,
				emitter,
				scope,
				ctx,
				loopStack
		...
		case "SwitchStatement":
			compileSwitchStatement(
				path as NodePath<t.SwitchStatement>,
				emitter,
				scope,
				ctx,
				loopStack
		...
		case "ThrowStatement":
			compileExpression(
				(path as NodePath<t.ThrowStatement>).get("argument"),
				emitter,
				scope,
				ctx
			);
		...
		case "TryStatement":
			compileTryStatement(
				path as NodePath<t.TryStatement>,
				emitter,
				scope,
				ctx,
				loopStack
		...
		case "LabeledStatement":
			compileLabeledStatement(
				path as NodePath<t.LabeledStatement>,
				emitter,
				scope,
				ctx,
				loopStack
		...
		case "ClassDeclaration":
			compileClassDeclaration(
				path as NodePath<t.ClassDeclaration>,
				emitter,
				scope,
				ctx
			);
...
export function compileDestructuringPattern(
	...
	if (pattern.isArrayPattern()) {
		...
		compileDestructuringPattern(
			pattern.get("argument") as NodePath<t.LVal>,
			emitter,
			scope,
			ctx,
			declKind
		...
		if (memberPath.node.computed) {
			compileExpression(
				memberPath.get("property") as NodePath<t.Expression>,
				emitter,
				scope,
				ctx
			);
			...
			const nameIdx = emitter.addStringConstant(
				(memberPath.node.property as t.Identifier).name
			);
		...
		if (prop.isRestElement()) {
			...
			for (const p2 of props) {
				...
				if (p2.isObjectProperty()) {
					...
					if (key.type === "Identifier" && !p2.node.computed)
						precedingKeys.push(key.name);
					else if (key.type === "StringLiteral")
						precedingKeys.push(key.value);
				}
			}
...
function compileIfStatement(
	...
	if (path.node.alternate) {
		...
		compileStatement(
			path.get("alternate") as NodePath<t.Statement>,
			emitter,
			scope,
			ctx,
			loopStack
...
function compileForStatement(
	...
	if (path.node.test) {
		compileExpression(
			path.get("test") as NodePath<t.Expression>,
			emitter,
			scope,
			ctx
		);
	...
	if (path.node.update) {
		compileExpression(
			path.get("update") as NodePath<t.Expression>,
			emitter,
			scope,
			ctx
		);
...
function compileForInStatement(
	...
	const needsIterScope =
		left.isVariableDeclaration() && left.node.kind !== "var";
	...
	if (left.isVariableDeclaration()) {
		...
		if (id.isIdentifier()) {
			...
			compileDestructuringPattern(
				...
				scope,
				ctx,
				left.node.kind as "var" | "let" | "const"
			);
		}
...
function compileSwitchStatement(
	...
	for (let i = 0; i < cases.length; i++) {
		...
		if (c.node.test) {
			emitter.emit(Op.DUP, 0);
			compileExpression(
				c.get("test") as NodePath<t.Expression>,
				emitter,
				scope,
				ctx
			);
...
function compileTryStatement(
	...
	if (hasCatch) {
		...
		if (handler.node.param) {
			...
			if (param.isIdentifier()) {
				...
				if (savedReg !== undefined)
					ctx.registerMap.delete(catchParamName);
			} else {
	...
	if (hasCatch && hasFinally) {
		...
		emitter.patchOperand(
			catchFinallyPushIdx,
			encodeTryTarget(-1, finallyIp)
		);
	} else {
		emitter.patchOperand(tryPushIdx, encodeTryTarget(catchIp, finallyIp));
	}
...
export function decodeTryTarget(encoded: number): {
	catchIp: number;
	finallyIp: number;
} {
...
function compileFunctionDeclaration(
	...
	if (reg !== undefined) {
		ctx.compileNestedFunction(
			path as unknown as NodePath<t.Function>,
			emitter,
			scope
		);
		emitter.emit(Op.STORE_REG, reg);
...
function compileClassDeclaration(
	...
	if (reg !== undefined) {
		ctx.compileClassExpression(
			path as unknown as NodePath<t.ClassExpression>,
			emitter,
			scope
		);
		emitter.emit(Op.STORE_REG, reg);
