...
function generateExecBody(
	...
	const awaitHandler = isAsync
		? `case ${Op.AWAIT}:{${
				debug
					? `${n.dbg}('AWAIT','awaiting:',typeof ${S}[${P}]==='object'?'[Promise]':${S}[${P}]);`
					: ""
		  }${S}[${P}]=await ${S}[${P}];break;}`
...
${fnDecl}(${U},${A},${OS},${TV},${NT},${HO}){
  ...
  if(${n.depth}>${VM_MAX_RECURSION_DEPTH}){${n.depth}--;${
		n.callStack
	}.pop();throw new RangeError('Maximum call '+'s'+'tack size exceeded');}
  try{
  var ${S}=[];
  var ${R}=new Array(${U}.r);
  var ${IP}=0;
  ...
  while(${IP}<_il){
    ...
    case ${
		Op.DUP2
	}:{var _a=${S}[${P}-1];var _b=${S}[${P}];${S}[++${P}]=_a;${S}[++${P}]=_b;break;}
    case ${
		Op.SWAP
	}:{var a=${S}[${P}--];var b=${S}[${P}--];${S}[++${P}]=a;${S}[++${P}]=b;break;}
    case ${
		Op.ROT3
	}:{var c=${S}[${P}--];var b=${S}[${P}--];var a=${S}[${P}--];${S}[++${P}]=c;${S}[++${P}]=a;${S}[++${P}]=b;break;}
    case ${
		Op.ROT4
	}:{var d=${S}[${P}--];var c=${S}[${P}--];var b=${S}[${P}--];var a=${S}[${P}--];${S}[++${P}]=d;${S}[++${P}]=a;${S}[++${P}]=b;${S}[++${P}]=c;break;}
    ...
    case ${
		Op.LOAD_ARG_OR_DEFAULT
	}:${W}(${O}<${A}.length&&${A}[${O}]!==void 0?${A}[${O}]:void 0);break;
    ...
    case ${Op.RETURN}:{var _rv=${S}[${P}--];${
		debug ? `${n.dbg}('RETURN','value=',_rv);` : ""
	}if(${EX}&&${EX}.length>0){var _h=${EX}[${EX}.length-1];if(_h.finallyIp>=0){${CT}=1;${CV}=_rv;${EX}.pop();${P}=_h.sp;${IP}=_h.finallyIp*2;break;}}return _rv;}
    case ${Op.RETURN_VOID}:{${
		debug ? `${n.dbg}('RETURN_VOID');` : ""
	}if(${EX}&&${EX}.length>0){var _h=${EX}[${EX}.length-1];if(_h.finallyIp>=0){${CT}=1;${CV}=void 0;${EX}.pop();${P}=_h.sp;${IP}=_h.finallyIp*2;break;}}return void 0;}
    case ${Op.THROW}:{var _te=${S}[${P}--];${
		debug ? `${n.dbg}('THROW','error=',_te);` : ""
	}throw _te;}
    case ${
		Op.RETHROW
    ...
    case ${
		Op.SET_PROP_STATIC
	}:{var val=${S}[${P}--];var obj=${S}[${P}];var k=${C}[${O}];try{obj[k]=val;}catch(_){Object.defineProperty(obj,k,{value:val,writable:true,configurable:true});}break;}
    case ${
		Op.GET_PROP_DYNAMIC
    ...
    case ${
		Op.GET_SUPER_PROP
	}:{var sp2=${HO}?Object.getPrototypeOf(${HO}):Object.getPrototypeOf(Object.getPrototypeOf(${TV}));var key=${O}>=0?${C}[${O}]:${X}();${W}(sp2?sp2[key]:void 0);break;}
    case ${
		Op.SET_SUPER_PROP
	}:{var val=${X}();var sp2=${HO}?Object.getPrototypeOf(${HO}):Object.getPrototypeOf(Object.getPrototypeOf(${TV}));var key=${O}>=0?${C}[${O}]:${X}();if(sp2)sp2[key]=val;${W}(val);break;}
    case ${
    ...
    case ${Op.PUSH_SCOPE}:case ${Op.PUSH_BLOCK_SCOPE}:case ${
		Op.PUSH_CATCH_SCOPE
	}:${SC}={${sPar}:${SC},${sV}:{}};break;
    case ${Op.POP_SCOPE}:${SC}=${SC}.${sPar}||${SC};break;
    case ${
		Op.TDZ_CHECK
	}:{var name=${C}[${O}];if(${SC}.${sTdz}&&${SC}.${sTdz}[name])throw new ReferenceError("Cannot access '"+name+"' before initialization");break;}
    case ${
		Op.TDZ_MARK
	}:{var name=${C}[${O}];if(${SC}.${sTdz})delete ${SC}.${sTdz}[name];break;}
    case ${
		Op.PUSH_WITH_SCOPE
	}:{var wObj=${X}();${SC}={${sPar}:${SC},${sV}:wObj};break;}
    case ${
		Op.DELETE_SCOPED
	}:{var name=${C}[${O}];var s=${SC};while(s){if(name in s.${sV}){${W}(delete s.${sV}[name]);break;}s=s.${sPar};}break;}
    ...
    case ${Op.CALL}:{
      ...
      ${
			debug
				? `${n.dbg}('CALL','fn=',typeof fn,'argc='+callArgs.length,fn&&fn.name?'name='+fn.name:'');if(typeof fn!=='function')${n.dbg}('CALL_ERR','NOT A FUNCTION:',fn,'${S} depth='+${P});`
				: ""
		}
    ...
    case ${Op.CALL_METHOD}:{
      ...
      ${
			debug
				? `${n.dbg}('CALL_METHOD','fn=',typeof fn,'recv=',typeof recv,'argc='+callArgs.length,fn&&fn.name?'name='+fn.name:'');if(typeof fn!=='function')${n.dbg}('CALL_METHOD_ERR','NOT A FUNCTION:',fn,'recv=',recv);`
				: ""
		}
    ...
    case ${Op.SUPER_CALL}:{
      ...
      ${
			debug
				? `${n.dbg}('SUPER_CALL','argc='+argc,'superProto=',!!superProto,'superCtor=',superProto&&typeof superProto.constructor);`
				: ""
		}
    ...
    case ${Op.DEFINE_METHOD}:{
      ...
      ${
			debug
				? `${n.dbg}('DEFINE_METHOD','name='+name,'static='+!!isStatic,'isCtor='+(name==='constructor'));`
				: ""
		}
    ...
    case ${Op.INIT_PRIVATE_ENV}:case ${Op.ADD_PRIVATE_BRAND}:case ${
		Op.CHECK_PRIVATE_BRAND
	}:break;
    case ${
		Op.EXTEND_CLASS
	}:{var superCls=${X}();var cls=${Y}();cls.prototype=Object.create(superCls.prototype);cls.prototype.constructor=cls;Object.setPrototypeOf(cls,superCls);break;}
    ...
    case ${Op.NEW_ARROW}:{
      ${W}((function(u,cs,ct){if(u.s){return async function(..._a){return ${
		n.execAsync
	}(u,_a,cs,ct);};}
      return function(..._a){return ${n.exec}(u,_a,cs,ct);};})(${
		n.load
	}(${C}[${O}]),${SC},${TV}));break;
    }
    case ${Op.NEW_ASYNC}:{
      ...
      var _tv=this;if(!u.st){if(_tv==null)_tv=globalThis;else{var _tt=typeof _tv;if(_tt!=="object"&&_tt!=="function")_tv=Object(_tv);}}
      return ${n.execAsync}(u,_a,cs,_tv);};})(${
		n.load
	}(${C}[${O}]),${SC}));break;
    }
    ...
    case ${
		Op.SET_FUNC_NAME
	}:{var fn=${Y}();try{Object.defineProperty(fn,'name',{value:${C}[${O}],configurable:true});}catch(e){}break;}
    case ${
		Op.SET_FUNC_LENGTH
	}:{var fn=${Y}();try{Object.defineProperty(fn,'length',{value:${O},configurable:true});}catch(e){}break;}
    case ${Op.BIND_THIS}:case ${Op.MAKE_METHOD}:break;
    case ${
		Op.PUSH_CLOSURE_VAR
	}:{var name=${C}[${O}];var s=${SC};while(s){if(name in s.${sV}){${W}(s.${sV}[name]);break;}s=s.${sPar};}break;}
    case ${
		Op.STORE_CLOSURE_VAR
	}:{var name=${C}[${O}];var val=${X}();var s=${SC};while(s){if(name in s.${sV}){s.${sV}[name]=val;break;}s=s.${sPar};}break;}
    ...
    case ${Op.CREATE_GENERATOR}:case ${Op.GENERATOR_RESUME}:case ${
		Op.GENERATOR_RETURN
	}:case ${Op.GENERATOR_THROW}:break;
    case ${Op.SUSPEND}:case ${Op.RESUME}:break;
    case ${Op.ASYNC_GENERATOR_YIELD}:case ${Op.ASYNC_GENERATOR_NEXT}:case ${
		Op.ASYNC_GENERATOR_RETURN
	}:case ${Op.ASYNC_GENERATOR_THROW}:break;
    case ${
		Op.CREATE_ASYNC_FROM_SYNC_ITER
	}:{var it=${X}();${W}({_iter:it,_done:false,_value:void 0});break;}

    case ${Op.CATCH_BIND_PATTERN}:{var err=${X}();${W}(err);break;}
    case ${
		Op.THROW_IF_NOT_OBJECT
	}:{var v=${Y}();if(typeof v!=='object'||v===null)throw new TypeError('Value is not an object');break;}
    case ${
		Op.THROW_REF_ERROR
	}:{throw new ReferenceError(${C}[${O}]||'not defined');}
    case ${Op.THROW_TYPE_ERROR}:{throw new TypeError(${C}[${O}]||'type error');}
    case ${
		Op.THROW_SYNTAX_ERROR
	}:{throw new SyntaxError(${C}[${O}]||'syntax error');}

    case ${Op.ITER_VALUE}:{var iterObj=${Y}();${W}(iterObj._value);break;}
    case ${
		Op.ITER_CLOSE
	}:{var iterObj=${X}();if(iterObj._iter.return)iterObj._iter.return();break;}
    case ${
		Op.ITER_RESULT_UNWRAP
	}:{var iterObj=${Y}();${W}(iterObj._value);${W}(!!iterObj._done);break;}
    ...
    case ${Op.ASYNC_ITER_NEXT}:{var iterObj=${Y}();var result=${
		isAsync ? "await iterObj._iter.next()" : "iterObj._iter.next()"
    ...
    case ${Op.ASYNC_ITER_CLOSE}:{var iterObj=${X}();if(iterObj._iter.return)${
		isAsync ? "await iterObj._iter.return()" : "iterObj._iter.return()"
	};break;}
    case ${Op.FOR_AWAIT_NEXT}:{var iterObj=${Y}();var result=${
		isAsync ? "await iterObj._iter.next()" : "iterObj._iter.next()"
	};iterObj._done=!!result.done;iterObj._value=result.value;${W}(result.value);break;}
    ...
    case ${
		Op.TO_PROPERTY_KEY
	}:{var v=${S}[${P}];${S}[${P}]=typeof v==='symbol'?v:String(v);break;}
    case ${
		Op.TO_NUMERIC
	}:{var v=${S}[${P}];${S}[${P}]=typeof v==='bigint'?v:Number(v);break;}
    ...
    case ${
		Op.INC_SCOPED
	}:{var name=${C}[${O}];var s=${SC};while(s){if(name in s.${sV}){s.${sV}[name]=s.${sV}[name]+1;${W}(s.${sV}[name]);break;}s=s.${sPar};}break;}
    case ${
		Op.DEC_SCOPED
	}:{var name=${C}[${O}];var s=${SC};while(s){if(name in s.${sV}){s.${sV}[name]=s.${sV}[name]-1;${W}(s.${sV}[name]);break;}s=s.${sPar};}break;}
    case ${
		Op.POST_INC_SCOPED
	}:{var name=${C}[${O}];var s=${SC};while(s){if(name in s.${sV}){var old=s.${sV}[name];s.${sV}[name]=old+1;${W}(old);break;}s=s.${sPar};}break;}
    case ${
		Op.POST_DEC_SCOPED
	}:{var name=${C}[${O}];var s=${SC};while(s){if(name in s.${sV}){var old=s.${sV}[name];s.${sV}[name]=old-1;${W}(old);break;}s=s.${sPar};}break;}
    case ${
		Op.ADD_ASSIGN_SCOPED
	}:{var val=${X}();var name=${C}[${O}];var s=${SC};while(s){if(name in s.${sV}){s.${sV}[name]+=val;${W}(s.${sV}[name]);break;}s=s.${sPar};}break;}
    case ${
    ...
    case ${
		Op.AND_ASSIGN_SCOPED
	}:{var val=${X}();var name=${C}[${O}];var s=${SC};while(s){if(name in s.${sV}){s.${sV}[name]=s.${sV}[name]&&val;${W}(s.${sV}[name]);break;}s=s.${sPar};}break;}
    case ${
		Op.OR_ASSIGN_SCOPED
	}:{var val=${X}();var name=${C}[${O}];var s=${SC};while(s){if(name in s.${sV}){s.${sV}[name]=s.${sV}[name]||val;${W}(s.${sV}[name]);break;}s=s.${sPar};}break;}
    case ${
		Op.NULLISH_ASSIGN_SCOPED
	}:{var val=${X}();var name=${C}[${O}];var s=${SC};while(s){if(name in s.${sV}){if(s.${sV}[name]==null)s.${sV}[name]=val;${W}(s.${sV}[name]);break;}s=s.${sPar};}break;}
    case ${Op.ASSIGN_OP}:break;
    case ${Op.INC_REG}:${R}[${O}]=+${R}[${O}]+1;break;
    case ${Op.DEC_REG}:${R}[${O}]=+${R}[${O}]-1;break;
    case ${
		Op.POST_INC_REG
	}:{var old=${R}[${O}];${R}[${O}]=+old+1;${W}(+old);break;}
    case ${
		Op.POST_DEC_REG
	}:{var old=${R}[${O}];${R}[${O}]=+old-1;${W}(+old);break;}
    case ${
		Op.ADD_ASSIGN_REG
	}:{var val=${S}[${P}--];${R}[${O}]=${R}[${O}]+val;${W}(${R}[${O}]);break;}
    case ${
		Op.SUB_ASSIGN_REG
	}:{var val=${S}[${P}--];${R}[${O}]=${R}[${O}]-val;${W}(${R}[${O}]);break;}
    case ${
    ...
    case ${
		Op.FAST_GET_PROP
	}:{var name=${C}[${O}&0xFFFF];var varName=${C}[(${O}>>16)&0xFFFF];var s=${SC};while(s){if(varName in s.${sV}){${W}(s.${sV}[varName][name]);break;}s=s.${sPar};}break;}
    ...
    case ${
		Op.PUSH_WELL_KNOWN_SYMBOL
	}:{var syms=[Symbol.iterator,Symbol.asyncIterator,Symbol.hasInstance,Symbol.toPrimitive,Symbol.toStringTag,Symbol.species,Symbol.isConcatSpreadable,Symbol.match,Symbol.replace,Symbol.search,Symbol.split,Symbol.unscopables];${W}(syms[${O}]||Symbol.iterator);break;}
    ...
    case ${
		Op.ASSERT_DEFINED
	}:{var v=${Y}();if(v===void 0)throw new TypeError('Cannot read properties of undefined');break;}
    case ${
		Op.ASSERT_FUNCTION
	}:{var v=${Y}();if(typeof v!=='function')throw new TypeError(v+' is not a function');break;}

    case ${Op.DESTRUCTURE_BIND}:break;
    case ${
		Op.DESTRUCTURE_DEFAULT
    ...
    case ${Op.CREATE_UNMAPPED_ARGS}:case ${
		Op.CREATE_MAPPED_ARGS
	}:${W}(Array.prototype.slice.call(${A}));break;
    case ${
		Op.CREATE_REST_ARGS
	}:${W}(Array.prototype.slice.call(${A},${O}));break;

    // --- Superinstructions (Tier 3) ---
    case ${
		Op.REG_ADD
	}:{var ra=${O}&0xFFFF;var rb=(${O}>>>16)&0xFFFF;${W}(${R}[ra]+${R}[rb]);break;}
    case ${
		Op.REG_SUB
	}:{var ra=${O}&0xFFFF;var rb=(${O}>>>16)&0xFFFF;${W}(${R}[ra]-${R}[rb]);break;}
    case ${
    ...
    case ${
		Op.REG_LT_CONST_JF
	}:{var r=${O}&0xFF;var ci=(${O}>>>8)&0xFF;var tgt=(${O}>>>16)&0xFFFF;if(!(${R}[r]<${C}[ci]))${IP}=tgt*2;break;}
    case ${
		Op.REG_GET_PROP
	}:{var r=${O}&0xFFFF;var ni=(${O}>>>16)&0xFFFF;${W}(${R}[r][${C}[ni]]);break;}
    case ${
		Op.REG_ADD_CONST
	}:{var r=${O}&0xFFFF;var ci=(${O}>>>16)&0xFFFF;${R}[r]=${R}[r]+${C}[ci];break;}
    case ${
		Op.REG_GTE
	}:{var ra=${O}&0xFFFF;var rb=(${O}>>>16)&0xFFFF;${W}(${R}[ra]>=${R}[rb]);break;}
    case ${
    ...
    case ${
		Op.REG_CONST_SUB
	}:{var r=${O}&0xFFFF;var ci=(${O}>>>16)&0xFFFF;${W}(${R}[r]-${C}[ci]);break;}
    case ${
		Op.REG_CONST_MUL
	}:{var r=${O}&0xFFFF;var ci=(${O}>>>16)&0xFFFF;${W}(${R}[r]*${C}[ci]);break;}
    case ${
    ...
    case ${
		Op.REG_LT_REG_JF
	}:{var ra=${O}&0xFF;var rb=(${O}>>>8)&0xFF;var tgt=(${O}>>>16)&0xFFFF;if(!(${R}[ra]<${R}[rb]))${IP}=tgt*2;break;}
 ...
 */
function filterUnusedOpcodeHandlers(
	template: string,
	usedOpcodes: Set<number>
): string {
...
function injectDecoyHandlers(
	...
	const decoys = selected
		.map((logicalOp, i) => {
			const body =
				decoyTemplates[(logicalOp + i) % decoyTemplates.length]!;
			return `case ${logicalOp}:${body}`;
		})
		.join("\n    ");

	// Insert decoys before `default:break;`
	return template.replace(/(\s*)(default:break;)/, `$1${decoys}\n    $2`);
}
