 ...
 */
export function generateStringDecoderSource(
	names: RuntimeNames,
	key: number,
	implicitKey: boolean = false
): string {
	if (implicitKey) {
	...
	return `function ${names.strDec}(b,x){var k=(${
		key >>> 0
	}^(x*0x9E3779B9))>>>0;var s='';for(var i=0;i<b.length;i++){k=(k*1664525+1013904223)>>>0;s+=String.fromCharCode(b[i]^(k&65535));}return s;}`;
}
