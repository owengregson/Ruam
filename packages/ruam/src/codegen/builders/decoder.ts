			...
			[
				returnStmt(
					newExpr(id("Uint8Array"), [
						call(member(id("Buffer"), "from"), [str, lit("base64")]),
					])
				),
			]
		),
...
function buildStrDecFunction(
	...
	const keySource: JsNode = implicit
		? id("mk")
		: lit(embeddedKey >>> 0);
	...
	const body: JsNode[] = [
		...
		varDecl(
			"k",
			u32(xor(keySource, bin("*", x, lit(0x9e3779b9))))
		),
		// var s = '';
		varDecl("s", lit("")),
		...
		forStmt(
			...
			[
				...
				exprStmt(
					assign(
						k,
						u32(bin("+", bin("*", k, lit(1664525)), lit(1013904223)))
					)
				),
				// s += String.fromCharCode(b[i] ^ (k & 65535));
				exprStmt(
