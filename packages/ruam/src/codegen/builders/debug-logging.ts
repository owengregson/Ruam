...
export function buildDebugLogging(
	...
	const dbgFn = fn(names.dbg, [], [
		...
		varDecl(
			"args",
			call(
				member(member(member(id("Array"), "prototype"), "slice"), "call"),
				[id("arguments")]
			)
		),
		// console.log.apply(console,['[VM_DBG]'].concat(args));
	...
	const dbgOpFn = fn(names.dbgOp, [OP, O, "C", P, S], [
		// if(!cfg.enabled||cfg.levels[cfg.level]>0)return;
		ifStmt(
			bin(
				"||",
				...
				assign(
					...
					ternary(
						...
						ternary(
							bin(
								"&&",
								bin("===", un("typeof", topVar), lit("object")),
								bin("!==", topVar, lit(null))
							),
								...
								bin(
									...
									call(
										member(
											call(
												member(
													call(member(id("Object"), "keys"), [topVar]),
													"slice"
												),
												[lit(0), lit(3)]
											),
			...
			ifStmt(bin(">", member(topStrVar, "length"), lit(60)), [
				exprStmt(
					assign(
						...
						bin(
							"+",
							call(member(topStrVar, "slice"), [lit(0), lit(60)]),
							lit("...")
						)
					)
				),
		...
		// if(typeof C[O]==='string')constStr=' c="'+C[O].slice(0,30)+'"';
		ifStmt(bin("===", un("typeof", cAtO), lit("string")), [
			exprStmt(
				assign(
					constStrVar,
					bin(
						...
						bin(
							"+",
							lit(' c="'),
							call(member(cAtO, "slice"), [lit(0), lit(30)])
						),
						lit('"')
					)
				)
