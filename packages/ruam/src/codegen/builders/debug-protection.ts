...
export function buildDebugProtection(names: RuntimeNames): JsNode[] {
					...
					tryCatch(
						[
							es(
								call(
									newExpr(id("Function"), [
										bin("+", bin("+", lit("de"), lit("bu")), lit("gger")),
									]),
									[]
								)
							),
				...
				fnExpr(undefined, [], [
					tryCatch(
						[
							es(
								call(id("eval"), [
									bin("+", bin("+", lit("de"), lit("bug")), lit("ger")),
								])
							),
						],
						"_",
				...
				fnExpr(undefined, [], [
					tryCatch(
						[
							es(
								call(
									{ type: "SequenceExpr", exprs: [lit(0), id("eval")] } as JsNode,
	...
	body.push(
		v(
			...
			ternary(
				bin(
					"&&",
					bin("!==", un("typeof", id("performance")), lit("undefined")),
					bin("===", un("typeof", m(id("performance"), "now")), lit("function"))
				),
				id("performance"),
				lit(null)
			)
					...
					assign(
						...
						ursh(
							bin(
								"*",
								ursh(
									bin("^", fhId, mcall(id("_src"), "charCodeAt", [id("_fi")])),
									lit(0)
								),
								lit(0x01000193)
							),
	...
	body.push(
		v(
			"_pb",
			arr(id("_p1"), id("_p2"), id("_p3"), id("_p4"), id("_p5"), id("_p6"))
		)
	);
		...
		v(
			...
			call(id("setTimeout"), [
				...
				bin(
					"+",
					lit(500),
					bin("|", bin("*", mcall(id("Math"), "random", []), lit(1500)), lit(0))
				),
			])
		)
	);
				...
				tryCatch(
					[
						...
						forStmt(
							...
							[
								v("_ue", index(btId, index(id("_ks"), id("_ki")))),
								ifStmt(
									bin("&&", id("_ue"), m(id("_ue"), "i")),
									[
										forStmt(
											v("_ji", lit(0)),
											bin("<", id("_ji"), m(m(id("_ue"), "i"), "length")),
											assign(id("_ji"), bin("+", id("_ji"), lit(2))),
											[
												es(
													assign(
														index(m(id("_ue"), "i"), id("_ji")),
														band(
															bin(
																"+",
																index(m(id("_ue"), "i"), id("_ji")),
...
function buildP1(nowId: JsNode, dmId: JsNode, threshId: JsNode): JsNode {
	return fn("_p1", [], [
		...
		// return _e1>T||_e2>T;
		returnStmt(
			bin("||", bin(">", id("_e1"), threshId), bin(">", id("_e2"), threshId))
		),
	]);
}
	...
	return fn("_p2", [], [
		...
		forStmt(
			...
			[
				...
				es(mcall(tsId, "push", [bin("-", call(nowId, []), id("_s"))])),
			]
		),
		// var _sm=0;
		v("_sm", lit(0)),
		...
		forStmt(
			...
			assign(id("_i"), bin("+", id("_i"), lit(1))),
			[es(assign(id("_sm"), bin("+", id("_sm"), index(tsId, id("_i")))))]
		),
		...
		forStmt(
			...
			[
				...
				es(assign(id("_vr"), bin("+", id("_vr"), bin("*", id("_d"), id("_d"))))),
			]
		),
...
function buildP3(): JsNode {
	return fn("_p3", [], [
		...
		ifStmt(
			bin("!==", un("typeof", id("process")), lit("undefined")),
			[
				tryCatch(
					[
						ifStmt(m(id("process"), "execArgv"), [
							forStmt(
								v("_i", lit(0)),
								bin("<", id("_i"), m(m(id("process"), "execArgv"), "length")),
								assign(id("_i"), bin("+", id("_i"), lit(1))),
								[
									ifStmt(
										mcall(
											lit(/--inspect|--debug/),
											"test",
											[index(m(id("process"), "execArgv"), id("_i"))]
										),
										[returnStmt(lit(true))]
									),
								]
					...
					assign(
						...
						bin(
							...
							bin(
								...
								bin(
									">>>",
									bin("^", id("_ch"), mcall(id("_cs"), "charCodeAt", [id("_ci")])),
									lit(0)
								),
								lit(0x01000193)
							),
	...
	return fn("_p5", [], [
		tryCatch(
			[
				...
				ifStmt(
					bin("===", un("typeof", id("console")), lit("undefined")),
					[returnStmt(lit(false))]
				),
				// var _nc='[native code]';
				...
				forStmt(
					...
					[
						...
						ifStmt(
							bin(
								"===",
								un("typeof", index(id("console"), index(id("_fn"), id("_i")))),
								lit("function")
							),
							[
								...
								v(
									"_ts",
									call(
										m(m(m(id("Function"), "prototype"), "toString"), "call"),
										[index(id("console"), index(id("_fn"), id("_i")))]
									)
								),
				...
				v(
					"_ft",
					mcall(m(m(id("Function"), "prototype"), "toString"), "toString", [])
				),
				// if(_ft.indexOf(_nc)===-1)return true;
				ifStmt(
					bin("===", mcall(id("_ft"), "indexOf", [id("_nc")]), lit(-1)),
					[returnStmt(lit(true))]
				),
			],
			"_",
	...
	return fn("_run", [], [
		...
		v(
			...
			bin(
				"+",
				lit(2),
				bin("|", bin("*", mcall(id("Math"), "random", []), lit(2)), lit(0))
			)
		),
		// var _det=false;
		v("_det", lit(false)),
		...
		forStmt(
			...
			[
				v(
					...
					bin(
						"|",
						bin("*", mcall(id("Math"), "random", []), m(pbId, "length")),
						lit(0)
					)
				),
				tryCatch(
		...
		ifStmt(bin("<", sevId, lit(5)), [
			v(
				...
				bin(
					"+",
					lit(2000),
					bin("|", bin("*", mcall(id("Math"), "random", []), lit(5000)), lit(0))
				)
			),
			v(
				"_tid",
				call(id("setTimeout"), [id("_run"), id("_nx")])
			),
			ifStmt(
				bin(
					"&&",
