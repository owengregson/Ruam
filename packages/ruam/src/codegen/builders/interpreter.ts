...
function buildScaffoldAST(
	...
	outerBody.push(
		ifStmt(
			bin(">", id(n.depth), lit(VM_MAX_RECURSION_DEPTH)),
			[
		...
		tryBody.push(
			exprStmt(
				call(id(n.dbg), [
					...
					bin("+", lit("async="), un("!", un("!", member(id(U), "s")))),
	...
	if (rollingCipher) {
		tryBody.push(
			varDecl(n.rcState, call(id(n.rcDeriveKey), [id(U)]))
		);
	}
	...
	// var _h=EX.pop()
	exHandlerBody.push(
		varDecl("_h", call(member(id(EX), "pop"), []))
	);
	...
	// P=_h._sp
	catchRouteBody.push(
		exprStmt(assign(id(P), member(id("_h"), "_sp")))
	);
	// Push error onto stack: S[++P]=e
	catchRouteBody.push(
		exprStmt(stackPush(S, P, id("e")))
	);
	// IP=_h._ci*2
	catchRouteBody.push(
		exprStmt(
			assign(id(IP), bin("*", member(id("_h"), "_ci"), lit(2)))
		)
	);
	// continue
	...
	finallyRouteBody.push(
		exprStmt(
			assign(id(IP), bin("*", member(id("_h"), "_fi"), lit(2)))
		)
	);
	// continue
...
function generateDecoyHandlers(
	...
	const decoyBodyFactories: (() => JsNode[])[] = [
		...
		() => [
			exprStmt(
				assign(index(id(S), id(P)), un("~", index(id(S), id(P))))
			),
		],
		// S[++P]=C[O]
		() => [
		...
		assign(
			...
			newExpr(id("Proxy"), [
				...
				obj(
					[
						"set",
						fnExpr(undefined, ["_", "k", "v"], setBody),
					],
					[
						"get",
						fnExpr(undefined, ["_", "k"], getBody),
					]
				),
			])
		)
